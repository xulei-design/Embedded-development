...
#include "delay.h"
#include <reg51.h> //要用到P口

#define LCD_1602_4PINS //如果声明了LCD1602_4PINS 代表使用4位并行接口方式，否则使用8位并行接口
...
void LCDWriteData(unsigned char dat); //变量中是ASCII码
void LCDInit(); //使用LCD1602首先要调用的
...
#define LCD_MODE_PIN8 0x38 //8位数据口,两行,5*8点阵 0011 1000
#define LCD_MODE_PIN4 0x28 //4位数据口,两行,5*8点阵 0010 1000

#define LCD_SCREEN_CLR 0x01 //清屏
#define LCD_CURSOT_RST 0x02 //光标复位

//显示开关控制指令
#define LCD_DIS_CUR_BLK_ON 0x0F //显示开,光标开,光标闪烁
#define LCD_DIS_CUR_ON     0x0E //显示开,光标看,光标不闪烁
...
/*** 
CGRAM地址设置，character generator RAM(0~3FH)
DB7 DB6 DB5 DB4 DB3 DB2 DB1 DB0
